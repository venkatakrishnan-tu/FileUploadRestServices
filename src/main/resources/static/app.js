...
app.service('UploadService', [ '$http', '$rootScope', function($http, $rootScope) {
	this.search = function(name, date) {
		$http.get("http://localhost:8080/rest/files", {
			params : {
				author : name,
				date : date
			}
...
app.service('fileUpload', ['$http','UploadService', function($http, UploadService) {
	this.uploadFileToUrl = function(uploadUrl, file, name, date) {
		...
		$http.post(uploadUrl, fd, {
			...
			UploadService.search(null, null);
		}).error(function() {
		});
	}
} ]);
...
app.run(function($rootScope, $http) {
	$http.get("http://localhost:8080/rest/files").success(
			function(response) {
				$rootScope.metadataList = response;
			});
});
